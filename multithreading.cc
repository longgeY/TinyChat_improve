...
void MatmulOperator::mat_mul_multithreading(struct matmul_params* params) {
    ...
    int m = C->row, n = C->column, k = A->column;
    // A: m x k; B: n x k; C: m x n
    // printf("A->row = %d, A->column = %d, B->row = %d, B->column = %d, C->row = %d, C->column = %d\n", A->row, A->column, B->row, B->column, C->row, C->column);
    ...
    for (int i = 0; i < num_thread; ++i) {
        ...
        int rc = pthread_create(&thread_pool[i], NULL, multithreading_worker_func, (void *)&threads_args[i]);
        if (rc) {
