        ...
        for (int col = 0; col < n; col++) {
            ...
            for (int q = 0; q < num_block; q++) {
                ...
                */
                ...
                const uint8x16_t lower_half_u8 = vandq_u8(w0, mask_low4bit);

                // (2) use `vshrq_n_u8` to right shift 4 bits and get the upper half
                const uint8x16_t upper_half_u8 = vshrq_n_u8(w0, 4); 

                // (3) use `vreinterpretq_s8_u8` to interpret the vector as int8
                const int8x16_t lower_half_s8 = vreinterpretq_s8_u8(lower_half_u8);  
                const int8x16_t upper_half_s8 = vreinterpretq_s8_u8(upper_half_u8); 
                ...
                const int8x16_t adjusted_lower_half = vsubq_s8(lower_half_s8, offsets); 
                const int8x16_t adjusted_upper_half = vsubq_s8(upper_half_s8, offsets);
                ...
                int_sum0 = vdotq_s32(int_sum0, a0, adjusted_lower_half); 
                int_sum0 = vdotq_s32(int_sum0, a1, adjusted_upper_half); 
